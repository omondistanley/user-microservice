...
async def test_net_worth_summary_aggregates_components(auth_override, monkeypatch):
    ...
    refresh = body.get("metadata", {}).get("refresh", {})
    assert isinstance(refresh.get("generated_at"), str)
    assert refresh.get("sources", {}).get("expense", {}).get("status") == "ok"
    assert refresh.get("sources", {}).get("investments", {}).get("status") == "ok"
...
async def test_net_worth_summary_ignores_cashflow_fields(auth_override, monkeypatch):
    ...
    refresh = body.get("metadata", {}).get("refresh", {})
    assert refresh.get("sources", {}).get("investments", {}).get("status") in {"not_configured", "unavailable"}
