...
(function() {
  ...
  function gatewayJson(path, options) {
    ...
    if (options.body && typeof options.body === "object" && !(options.body instanceof FormData)) {
      ...
      options = Object.assign({}, options, { headers, body: JSON.stringify(options.body) });
    } else {
  ...
  function fetchQuote(symbol) {
    return gatewayJson("/api/v1/market/quote/" + encodeURIComponent(symbol)).catch(function() {
      return null;
    });
  }
  function fetchAllQuotes(symbols) {
    var unique = symbols.filter(function(s, i, a) {
      return a.indexOf(s) === i;
    });
    return Promise.all(unique.map(function(s) {
      return fetchQuote(s).then(function(q) {
        return { symbol: s, quote: q };
      });
    })).then(function(results) {
      var map = {};
      results.forEach(function(r) {
        map[r.symbol] = r.quote;
      });
      return map;
    });
  }
  ...
  function fetchAlpacaStatus() {
    return gatewayJson("/api/v1/alpaca/status").catch(function() {
  ...
  function placeOrder(symbol, qty, side) {
    return gatewayJson("/api/v1/alpaca/orders", {
      method: "POST",
      body: { symbol, qty: Number(qty), side, type: "market" }
    });
  }
  function sellHolding(symbol, quantity) {
    ...
    return placeOrder(symbol, quantity, "sell").then(function() {
      return alpacaSync().catch(function() {
      }).then(function() {
      ...
      if (listEl) {
        var symbols = items.map(function(h) {
  ...
  function renderGainsChart(data, visible) {
    ...
    if (series.length === 0) {
      ...
      if (series.length === 0) {
        series.push({ name: "Total", data: data.dates.map(function() {
          return 0;
        }) });
      }
    ...
    var opts = {
      ...
      series,
      xaxis: { categories: data.dates, labels: { rotate: -45, formatter: function(v) {
  ...
  function renderHoldings(listEl, items, quotes) {
    ...
    items.forEach(function(h) {
      ...
      var dayChangeDollars = price != null && changePct != null ? marketValue * (changePct / 100) / (1 + changePct / 100) : 0;
      ...
      html += '<li class="holding-item" data-holding-id="' + escapeHtml(h.holding_id || "") + '" data-symbol="' + escapeHtml(sym) + '" data-quantity="' + qty + '" data-source="' + escapeHtml(source) + '" data-cost="' + cost + '" data-price="' + (price != null ? price : "") + '" data-change-pct="' + (changePct != null ? changePct : "") + '" data-role="' + escapeHtml(roleLabel) + '" data-unrealized="' + (unrealized != null ? unrealized : "") + '">';
  ...
  function init() {
    ...
    document.querySelectorAll(".inv-period-btn").forEach(function(btn) {
      btn.addEventListener("click", function() {
        document.querySelectorAll(".inv-period-btn").forEach(function(b) {
          b.classList.remove("inv-period-btn--active");
        });
    ...
    document.querySelectorAll(".inv-chart-type-btn").forEach(function(btn) {
      btn.addEventListener("click", function() {
        document.querySelectorAll(".inv-chart-type-btn").forEach(function(b) {
          b.classList.remove("inv-chart-type-btn--active");
        });
    ...
    if (form) {
      form.addEventListener("submit", function(e) {
        ...
        var payload = { symbol, quantity, avg_cost: avgCost, currency: currency || "USD", account_type: accountType };
            ...
            return fetchAlpacaStatus().then(function(status) {
              if (status && status.connected && confirm("Place this as a buy order on Alpaca? Market order for " + quantity + " share(s) of " + symbol.toUpperCase() + ".")) {
                return placeOrder(symbol.toUpperCase(), quantity, "buy").then(function() {
                  ...
                  if (d && d.items) {
                    var syms2 = d.items.map(function(h) {
                      return (h.symbol || "").toUpperCase();
                    });
  ...
  function fetchPortfolioHealth() {
    return gatewayJson("/api/v1/portfolio/health").catch(function() {
      return null;
    });
  }
  ...
  var NORTH_STAR_TTL_MS = 5 * 60 * 1e3;
  ...
  function writeNorthStarCache(data) {
    try {
      sessionStorage.setItem(NORTH_STAR_CACHE_KEY, JSON.stringify({ ts: Date.now(), data }));
    } catch (e) {
    }
  }
  ...
  function fetchNorthStarBundle() {
    ...
    return Promise.all([
      gatewayJson("/api/v1/risk-profile").catch(function() {
        return {};
      }),
      gatewayJson("/api/v1/portfolio/sector-breakdown").catch(function() {
        return null;
      }),
      gatewayJson("/api/v1/recommendations/latest?page=1&page_size=1").catch(function() {
        return null;
      }),
      fetchSurplus()
    ]).then(function(parts) {
  ...
  function fetchHoldingDetail(symbol) {
    return gatewayJson("/api/v1/holdings/by-symbol/" + encodeURIComponent(symbol) + "/detail").catch(function() {
      return null;
    });
  }
      ...
      if (isHidden) {
        ...
        if (!loaded) {
          ...
          gatewayJson("/api/v1/holdings").then(function(data) {
            ...
            if (!items.length) {
              if (list) list.innerHTML = '<p class="muted">Add holdings to see dividend information.</p>';
              return;
            }
            var rows = items.filter(function(h) {
              return h.symbol;
            });
  ...
  function checkCashOpportunity() {
    ...
    fetchSurplus().then(function(data) {
      ...
      if (surplus > 500) {
        ...
        text.textContent = "Your estimated available amount of $" + Math.round(surplus).toLocaleString() + " could earn approximately $" + hysa_low + "\u2013$" + hysa_high + "/year in a high-yield savings account (vs. a typical bank rate). This is informational \u2014 actual rates vary.";
        strip.style.display = "block";
      }
    });
  }
