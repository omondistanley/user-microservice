...
(function() {
  ...
  window.TellerApi = {
    ...
    deleteEnrollment: function(enrollmentId) {
      ...
      var doFetch = window.Auth && window.Auth.requestWithRefresh ? function() {
        return window.Auth.requestWithRefresh(API + "/api/v1/teller/enrollments/" + encodeURIComponent(enrollmentId), { method: "DELETE", headers });
      } : function() {
        return fetch(API + "/api/v1/teller/enrollments/" + encodeURIComponent(enrollmentId), { method: "DELETE", headers });
      };
      return doFetch().then(function(r) {
        if (!r.ok) throw new Error(r.statusText || "Request failed");
        return r.json().catch(function() {
