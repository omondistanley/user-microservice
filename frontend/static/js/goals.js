      ...
      return this.list().then(function(goals) {
        ...
        return Promise.all(goals.map(function(g) {
          ...
          withProgress.forEach(function(x) {
            ...
            html += '<div class="goal-card goal-card-budget" role="button" tabindex="0" data-goal-id="' + escapeHtml(goalId) + '" style="cursor:pointer;"><div style="display:flex;align-items:flex-start;gap:10px;margin-bottom:12px;"><div class="goal-icon" style="background:var(--brand-50,#eff6ff);color:var(--brand,#2563eb);">\u2B50</div><div style="flex:1;"><div class="goal-name">' + name + "</div>" + (g.target_date ? '<div class="goal-target">Target: ' + fmtMoney(target) + " \xB7 " + escapeHtml(String(g.target_date).slice(0, 10)) + "</div>" : '<div class="goal-target">Target: ' + fmtMoney(target) + "</div>") + '</div><span class="badge badge-emerald">' + pct + '%</span></div><div class="goal-progress-meta"><span class="goal-current">' + fmtMoney(current) + '</span><span class="goal-pct">' + pct + '%</span></div><div class="progress"><div class="progress-bar ' + barColor + '" style="width:' + pct + '%;"></div></div><div style="font-size:11px;color:var(--text-muted);margin-top:6px;">' + leftText + '</div><div style="display:flex;justify-content:flex-end;margin-top:12px;"><button type="button" class="btn btn-ghost btn-sm goal-edit-btn" data-goal-id="' + escapeHtml(goalId) + '" aria-label="Edit goal">Edit</button></div></div>';
          });
