...
(function() {
  ...
  window.Goals = {
    ...
    _lastRender: null,
    list: function() {
      return fetch(API + "/api/v1/goals", { headers: authHeaders() }).then(function(r) {
        return r.json();
      }).then(function(data) {
    ...
    getProgress: function(goalId) {
      return fetch(API + "/api/v1/goals/" + encodeURIComponent(goalId) + "/progress", { headers: authHeaders() }).then(function(r) {
        return r.json();
      }).then(function(data) {
        if (data.detail) throw new Error(data.detail);
        return data;
    ...
    updateGoal: function(goalId, payload) {
      ...
      return fetch(API + "/api/v1/goals/" + encodeURIComponent(goalId), {
        ...
        if (!r.ok) return r.json().then(function(d) {
          throw new Error(d.detail || "Failed to update goal");
        });
        return r.json();
      });
    },
    ...
    renderList: function(containerId, options) {
      ...
      this._lastRender = { containerId, options };
      ...
      return this.list().then(function(goals) {
        ...
        return Promise.all(goals.map(function(g) {
          return self.getProgress(g.goal_id).then(function(p) {
            ...
            return { goal: g, progress: null };
          });
        })).then(function(withProgress) {
          function fmtMoney(n) {
            var v = Number(n) || 0;
            return "$" + v.toLocaleString(void 0, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
          }
          var html = "";
          withProgress.forEach(function(x) {
            ...
            html += '<div class="card goal-card-budget" role="button" tabindex="0" data-goal-id="' + escapeHtml(goalId) + '" style="text-decoration:none;color:inherit;display:block;cursor:pointer;"><div class="goal-card-budget-inner" style="display:flex;flex-direction:column;"><div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;"><div style="display:flex;align-items:center;gap:10px;"><div style="width:40px;height:40px;border-radius:10px;background:var(--s100);display:flex;align-items:center;justify-content:center;font-size:18px;">\u2B50</div><span style="font-size:14px;font-weight:700;color:var(--text-primary);">' + name + '</span></div><span class="badge badge-emerald">' + pct + '%</span></div><div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:8px;"><span style="font-size:18px;font-weight:800;color:var(--text-primary);">' + fmtMoney(current) + '</span><span style="font-size:12px;color:var(--text-muted);">of ' + fmtMoney(target) + '</span></div><div class="progress-bar"><div class="progress-fill ' + fillClass + '" style="width:' + pct + '%;"></div></div><div style="font-size:12px;font-weight:500;margin-top:6px;color:var(--text-secondary);">' + leftText + '</div><div style="display:flex;justify-content:flex-end;margin-top:12px;"><button type="button" class="btn btn-ghost btn-sm goal-edit-btn" data-goal-id="' + escapeHtml(goalId) + '" aria-label="Edit goal">Edit</button></div></div></div>';
          });
          ...
          if (options.onTotals) options.onTotals({ totalSaved, totalTarget, count: goals.length });
        });
      }).catch(function(err) {
