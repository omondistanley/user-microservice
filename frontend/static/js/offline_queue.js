...
(function() {
  ...
  function enqueue(type, payload) {
    var id = generateId();
    var item = { id, type, payload, status: "pending", createdAt: (/* @__PURE__ */ new Date()).toISOString() };
    var q = getQueue();
    q.push(item);
    setQueue(q);
    return id;
  ...
  function removeFromQueue(id) {
    var q = getQueue().filter(function(x) {
      return x.id !== id;
    });
    setQueue(q);
  }
  function replayOne(item) {
    ...
    return fetch(url, { method: "POST", headers, body: JSON.stringify(item.payload) }).then(function(r) {
      if (r.ok) {
  ...
  function replayAll() {
    var pending = getQueue().filter(function(x) {
