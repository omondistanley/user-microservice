  ...
  window.PlaidApi = {
    ...
    getHostedLink: function(completionRedirectUri) {
      return request("/api/v1/plaid/link-hosted", {
        method: "POST",
        body: completionRedirectUri ? { completion_redirect_uri: completionRedirectUri } : void 0
      });
    },
    ...
    deleteItem: function(itemId) {
      ...
      var doFetch = window.Auth && window.Auth.requestWithRefresh ? function() {
        return window.Auth.requestWithRefresh(API + "/api/v1/plaid/items/" + encodeURIComponent(itemId), { method: "DELETE", headers });
      } : function() {
        return fetch(API + "/api/v1/plaid/items/" + encodeURIComponent(itemId), { method: "DELETE", headers });
      };
      return doFetch().then(function(r) {
        if (!r.ok) throw new Error(r.statusText || "Request failed");
        return r.json().catch(function() {
