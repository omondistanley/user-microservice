...
(function() {
  ...
  function notificationItemsHtml(items) {
    ...
    return items.map(function(item) {
      ...
      return '<div class="notification-item ' + stateClass + '" data-notification-id="' + item.notification_id + '"><div class="notification-item-title">' + (item.title || "Notification") + '</div><div class="notification-item-body">' + (item.body || "") + '</div><div class="notification-item-meta">' + createdAt + "</div></div>";
    }).join("");
  }
  ...
  function bindEvents() {
    ...
    function bindListReadHandler(el) {
      ...
      el.addEventListener("click", function(e) {
        ...
        request("/api/v1/notifications/" + encodeURIComponent(notificationId) + "/read", { method: "PATCH" }).then(function() {
          refresh();
        }).catch(function() {
        });
      });
  ...
  window.Notifications = {
    ...
    start: function() {
      ...
      bindEvents();
      refresh();
      pollHandle = window.setInterval(refresh, 3e4);
    },
    stop: function() {
      if (pollHandle) {
