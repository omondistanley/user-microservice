...
(function() {
  ...
  function render(d) {
    ...
    if (!el || !d) return;
    el.innerHTML = "<p><strong>Amount</strong> $" + esc(d.amount) + "</p><p><strong>Cadence</strong> " + esc(d.cadence) + "</p><p><strong>Category</strong> " + esc(d.category_code) + " \u2014 " + esc(d.category_name) + "</p><p><strong>Next period</strong> " + esc(d.next_period_start) + "</p><p><strong>Active</strong> " + (d.is_active ? "Yes" : "No") + "</p>";
  }
  function esc(s) {
    if (s == null) return "";
  ...
  function load() {
    if (!rid) return;
    fetch(API + "/api/v1/recurring-budgets/" + encodeURIComponent(rid), { headers: headers(false) }).then(function(r) {
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    del.addEventListener("click", function() {
      ...
      fetch(API + "/api/v1/recurring-budgets/" + encodeURIComponent(rid), { method: "DELETE", headers: headers(false) }).then(function(r) {
        if (r.status === 204) window.location.href = "/budgets/recurring";
        else if (msg) msg.textContent = "Delete failed.";
      }).catch(function() {
        if (msg) msg.textContent = "Delete failed.";
      });
    });
  });
})();
