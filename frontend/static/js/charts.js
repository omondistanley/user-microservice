...
(function() {
  ...
  function formatAmountLabel(val) {
    ...
    return getCurrencySymbol() + n.toLocaleString(void 0, { maximumFractionDigits: 0 });
  }
  ...
  window.ExpenseCharts = {
    renderCategoryChart: function(containerId, summaryItems, options2) {
      ...
      if (!wrapper) return;
      destroyExisting(container);
      var series = summaryItems.map(function(i) {
        return Number(i.total_amount);
      });
      var labels = summaryItems.map(function(i) {
        return i.label || i.group_key || "";
      });
    ...
    renderMonthlyTrendChart: function(containerId, summaryItems, options2) {
      var container = document.getElementById(containerId);
      if (!container) return;
      if (!summaryItems || summaryItems.length === 0) {
        showEmptyMessage(container, "No data for this range");
      ...
      var seriesData = sorted.map(function(i) {
        return Number(i.total_amount);
      });
      var wrapper = ensureChartDiv(container);
      if (!wrapper) return;
      ...
      var opts = Object.assign({}, defaultChartOptions(), {
        ...
        xaxis: { categories },
        yaxis: { title: { text: "Amount" }, labels: { formatter: function(v) {
          return formatAmountLabel(v);
        } } },
    ...
    renderBudgetVsSpentChart: function(containerId, budgetItemsWithSpent) {
      ...
      var categories = budgetItemsWithSpent.map(function(b) {
        return b.label || "";
      });
      var budgetSeries = budgetItemsWithSpent.map(function(b) {
        return Number(b.budgetAmount);
      });
      var spentSeries = budgetItemsWithSpent.map(function(b) {
        return Number(b.spentAmount);
      });
      var wrapper = ensureChartDiv(container);
      if (!wrapper) return;
      destroyExisting(container);
    ...
    renderBalanceOverTimeChart: function(containerId, balanceHistoryItems) {
      ...
      var opts = Object.assign({}, defaultChartOptions(), {
        ...
        chart: Object.assign({ type: "line", zoom: { enabled: false } }, defaultChartOptions().chart),
        xaxis: { categories },
        yaxis: { title: { text: "Balance" }, labels: { formatter: function(v) {
          return formatAmountLabel(v);
        } } },
        stroke: { curve: "smooth", width: 2 },
        dataLabels: { enabled: false }
