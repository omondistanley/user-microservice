/**
 * Budgets — list/add/detail via budget-microservice API.
 * Uses BUDGET_API_BASE when set (e.g. http://localhost:3002), else API_BASE/same origin.
 */
(function() {
    'use strict';
    var API = window.BUDGET_API_BASE !== undefined && window.BUDGET_API_BASE !== ''
        ? window.BUDGET_API_BASE
        : (window.API_BASE || '');

    // Category codes 1-8 align with expense-microservice
    var CATEGORIES = [
        { code: 1, name: 'Food' },
