...
(function () {
    ...
    function renderShapBars(contributions) {
        ...
        }).filter(function (e) { return Math.abs(e.val) > 0.0001; });
    ...
    function openExplainDrawer(rootEl, runId, symbol) {
        ...
        fetchExplainSymbol(runId, symbol).then(function (data) {
            ...
            // SHAP attribution bars — shown first so users immediately see what drove the rank
            var fc = (ex.score_breakdown && ex.score_breakdown.factor_contributions) || ex.factor_contributions;
            var shapHtml = renderShapBars(fc);
            if (shapHtml) {
                html += shapHtml;
                if (ex.model_version) {
                    html += '<p class="muted" style="font-size:0.75rem;margin-top:-0.5rem;margin-bottom:1rem;">Model: ' + escapeHtml(String(ex.model_version)) + ' · Not financial advice.</p>';
                }
            }
            if (ex.security) {
                var sec = ex.security;
    ...
    function openScoreModal(runId, symbol) {
        ...
        fetchExplainSymbol(runId, symbol).then(function (data) {
            ...
            if (!breakdown) {
                bodyEl.innerHTML = disclaimer + '<p class="muted">No score breakdown available.</p>' +
                    ...
                    '<p class="rec-score-modal-meta"><a href="/settings/integrations">Link accounts</a> under Settings so cashflow and goals can refine context when you enable that option.</p>';
                return;
            }
            var html = disclaimer;
            ...
            keys.forEach(function (k) {
                if (breakdown[k] == null || breakdown[k] === undefined) return;
            ...
            if (ex && ex.data_freshness) {
                ...
                if (ex.data_freshness.stale_seconds != null) html += ' · stale ~' + escapeHtml(String(ex.data_freshness.stale_seconds)) + 's';
                html += '</div>';
            }
            if (ex && ex.news_provider_status && ex.news_provider_status.configured_keys) {
                var ck = ex.news_provider_status.configured_keys;
                var bits = [];
                Object.keys(ck).forEach(function (k) { bits.push(k + '=' + (ck[k] ? 'on' : 'off')); });
        ...
        if (runBtn) {
            runBtn.addEventListener('click', function () {
                ...
                runRecommendations().then(function (data) {
                    ...
                    try { sessionStorage.removeItem(_NORTH_STAR_KEY); } catch (e) {}
    ...
    function fetchSurplusForStrip() {
        ...
        var base = gw || expBase || '';
        return fetch(base + '/api/v1/surplus', { headers: getAuthHeaders() })
            .then(function (r) { return r.ok ? r.json() : null; })
            .catch(function () { return null; });
    }

    var _NORTH_STAR_KEY = 'pocketii_north_star_v1';
    var _NORTH_STAR_TTL = 5 * 60 * 1000;
    ...
    function fetchNorthStarBundleRec() {
        ...
        return Promise.all([
            fetch(API + '/api/v1/risk-profile', { headers: h }).then(function (r) { return r.ok ? r.json() : {}; }),
            fetch(API + '/api/v1/portfolio/sector-breakdown', { headers: h }).then(function (r) { return r.ok ? r.json() : null; }),
            fetch(API + '/api/v1/recommendations/latest?page=1&page_size=1', { headers: h }).then(function (r) { return r.ok ? r.json() : null; }),
            ...
            try { sessionStorage.setItem(_NORTH_STAR_KEY, JSON.stringify({ ts: Date.now(), data: data })); } catch (e2) {}
            return data;
        });
    }
    ...
    function renderNorthStarStripRec() {
        ...
        }).catch(function () { if (strip) strip.style.display = 'none'; });
    }
        ...
        el.querySelectorAll('.watchlist-remove').forEach(function(btn) {
            btn.addEventListener('click', function() {
                ...
                fetch(API + '/api/v1/watchlist/' + id, { method: 'DELETE', headers: getAuthHeaders() })
                    ...
                    .catch(function() { showToast('Failed to remove from watchlist', 'error'); });
            });
        });
    }
        ...
        if (form) {
            form.addEventListener('submit', function(e) {
                ...
                fetch(API + '/api/v1/watchlist', {
                  ...
                  }).catch(function() { showToast('Failed to add to watchlist', 'error'); });
            });
        }
    }
    ...
    function renderMonteCarlo(data) {
        ...
        if (chartEl && typeof ApexCharts !== 'undefined') {
            ...
            // Build per-month percentile arrays if percentile_paths available, otherwise derive from sample_paths
            ...
            if (!p50arr && data.sample_paths && data.sample_paths.length) {
                // derive per-step percentiles from sample paths
                var paths = data.sample_paths;
                var nSteps = paths[0].length;
                p10arr = []; p25arr = []; p50arr = []; p75arr = []; p90arr = [];
                for (var si = 0; si < nSteps; si++) {
                    var col = paths.map(function(p) { return p[si]; }).sort(function(a,b){return a-b;});
                    var idx10 = Math.floor(col.length * 0.10);
                    var idx25 = Math.floor(col.length * 0.25);
                    var idx50 = Math.floor(col.length * 0.50);
                    var idx75 = Math.floor(col.length * 0.75);
                    var idx90 = Math.floor(col.length * 0.90);
            ...
            _mcChartInstance = new ApexCharts(chartEl, {
                ...
                yaxis: { labels: { formatter: function(v) { return '$' + Math.round(v/1000) + 'k'; }, style: { fontSize: '11px' } } },
                legend: { position: 'top', fontSize: '11px', markers: { radius: 3 } },
                dataLabels: { enabled: false },
                tooltip: { shared: true, y: { formatter: function(v) { return '$' + Math.round(v).toLocaleString(); } } },
