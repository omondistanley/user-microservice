...
(function () {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        fetch(API + '/api/v1/analytics/overview?days=30', { headers: headers() })
            .then(function (r) {
            ...
            .then(function (d) {
                ...
                if (ul) {
                    ul.innerHTML = rows.length ? rows.map(function (c) {
                        return '<li class="flex justify-between py-2 border-b border-slate-100 dark:border-slate-800">' +
                            '<span>' + esc(c.label || c.category_code) + '</span>' +
                            '<span class="font-semibold">$' + fmt(c.total) + '</span></li>';
