...
(function () {
        ...
        fetch(url, { headers: headers() })
            ...
            .then(function (data) {
                ...
                tbody.innerHTML = items.map(function (row) {
                    ...
                    var link = row.entry_type === 'expense'
                        ? '<a href="/expenses/' + encodeURIComponent(row.entry_id) + '" class="text-[#135bec] hover:underline">' + esc(desc) + '</a>'
                        : esc(desc);
                    return '<tr class="border-t border-slate-100 dark:border-slate-800">' +
                        '<td class="px-4 py-3 capitalize">' + esc(row.entry_type || '') + '</td>' +
                        '<td class="px-4 py-3 whitespace-nowrap">' + esc(row.occurred_on || '') + '</td>' +
                        '<td class="px-4 py-3">' + link + '</td>' +
                        '<td class="px-4 py-3 text-right font-semibold ' + cls + '">' + sign + '$' + Math.abs(amt).toFixed(2) + '</td></tr>';
                ...
                if (pag) {
                    pag.innerHTML = '<span class="text-sm text-slate-500">Page ' + page + ' / ' + pages + '</span>' +
                        (page > 1 ? ' <button type="button" class="tx-prev px-3 py-1 rounded border text-sm">Prev</button>' : '') +
                        (page < pages ? ' <button type="button" class="tx-next px-3 py-1 rounded border text-sm">Next</button>' : '');
                    var prev = pag.querySelector('.tx-prev');
                    var next = pag.querySelector('.tx-next');
                    if (prev) prev.onclick = function () { page--; load(); };
                    if (next) next.onclick = function () { page++; load(); };
                }
            })
            .catch(function () {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (r) r.addEventListener('click', function () { page = 1; load(); });
        var et = document.getElementById('tx-entry-type');
        if (et) et.addEventListener('change', function () { page = 1; load(); });
