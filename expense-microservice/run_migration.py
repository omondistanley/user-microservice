...
DB_NAME = os.environ.get("DB_NAME", os.environ.get("EXPENSE_DB_NAME", "expenses_db"))
...
def main():
    ...
    def _split_sql(text: str) -> list:
        ...
        A plain sql.split(";") breaks plpgsql functions that contain semicolons
        inside their body.
        """
        statements = []
        current = []
        ...
        i = 0
        while i < len(text):
            # Detect start/end of a dollar-quote block
            if text[i] == "$":
    ...
    def strip_comments(chunk: str) -> str:
        ...
        for line in chunk.splitlines():
            ...
            if stripped and not stripped.startswith("--"):
                lines.append(line)
        out = "\n".join(lines).strip()
        # Drop leading fragment lines left from splitting on ";" inside a comment
